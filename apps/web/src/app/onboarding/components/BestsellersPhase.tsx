...
export default function BestsellersPhase({ onNext, onBack }: BestsellersPhaseProps) {
  ...
  function toggle(idx: number) {
    const isSelected = items[idx].star_rating === SELECTED_VALUE;
    if (isSelected) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="mb-4">
        ...
        <p className="mt-1 text-xs text-slate-500 leading-relaxed">
          Pick up to {MAX_SELECT} of your bestsellers. We&apos;ll feature them at the top of your menu.
        </p>
      </div>
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        {selectedCount > 0 && (
          <button
            type="button"
            onClick={() => items.forEach((_, idx) => updateItemField(idx, 'star_rating', DEFAULT_VALUE))}
      ...
      <div className="flex-1 overflow-y-auto -mx-1 px-1 pb-2">
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            {items.map((item, idx) => {
              const isSelected = item.star_rating === SELECTED_VALUE;
              ...
              return (
                ...
                >
                  {/* Top row: food-type dot + checkmark */}
