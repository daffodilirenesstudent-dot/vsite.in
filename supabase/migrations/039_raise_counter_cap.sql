-- 039: Raise counter daily cap (99 → 999) + format C01 → C001
--
-- WHY: Closes P15 from the B-series audit. A malicious actor (or competitor)
-- placing 99 counter orders quickly could exhaust the daily counter capacity
-- and cause `counter_full` 503s for legitimate walk-ins until midnight.
--
-- Bumping to 999 turns the attack from "₹10K cost = day-long DoS" into
-- "₹100K cost = day-long DoS" — effectively infeasible for ordinary abusers.
--
-- COSMETIC: All counter numbers from now on print as 3-digit (C001 instead of
-- C01). Existing rows are unaffected — only NEW orders use the new format.
--
-- This migration re-publishes process_order_v2 verbatim from 034 with only the
-- two value changes:
--   c_max_counter_day  CONSTANT INT     := 99   → 999
--   lpad(v_seq::TEXT, 2, '0')                   → lpad(v_seq::TEXT, 3, '0')
--
-- The original 034 ALTER TABLE for kot_mode is INTENTIONALLY KEPT — IF NOT EXISTS
-- makes it a no-op on re-run.
