-- 028: Wrap order-number generation in a retry loop inside process_order_v2
--
-- The original code generates a single random 7-digit number per order.
-- With a UNIQUE index on orders.order_number (added in 026), a collision
-- will cause create_order_atomic to return NULL. This migration patches
-- process_order_v2 to retry up to 5 times on collision before giving up.
--
-- The retry loop is O(1) in the common case (collision probability < 0.01%
-- at even 1000 orders/day) and eliminates silent duplicate order numbers.
...
BEGIN
  ...
  -- ── 1. Rate limiting ──────────────────────────────────────────────────────
  -- IP limit: strict pre-check + immediate increment. One attacker can only
  -- exhaust their own IP quota — does not affect other customers.
  SELECT public.check_rate_limit(p_ip_rate_key, p_rl_window_ms, p_rl_ip_limit)
    INTO v_rl_ip_ok;

  -- Site limit: read-only check here. Increment happens AFTER successful
  -- order creation (see bottom of function). This prevents a competitor from
  -- deliberately sending 100 bad requests to lock out ALL customers at a site.
  ...
  -- With the UNIQUE index on orders.order_number (added in migration 026),
  -- create_order_atomic returns NULL on a duplicate. We retry up to 5 times
  -- with a fresh random number each time. Collision probability per attempt
  -- is < 0.01% at 1000 orders/day, so 5 retries is effectively infinite.
  ...
  -- Increment AFTER successful order creation so failed/abusive requests
  -- do not consume the site quota (see migration 029 for split design).
